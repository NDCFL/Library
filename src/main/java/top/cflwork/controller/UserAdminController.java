...
@Controller
@Api(value = "/userAdmin",description = "用户模块")
...
//	@RequiresPermissions("userAdmin:userPage")
	...
	@RequiresPermissions("userAdmin:add")
	@Log("添加用户")
	@GetMapping("/add")
	public String add(Model model) {
		List<RoleVo> roles = roleService.list(new HashMap<>(16));
		model.addAttribute("roles", roles);
		return prefix + "/add";
	}

	@RequiresPermissions("userAdmin:edit")
	@Log("编辑用户")
	@GetMapping("/edit/{id}")
	...
	@RequiresPermissions("userAdmin:remove")
	@Log("删除用户")
	@PostMapping("/remove")
	@ResponseBody
	...
	@RequiresPermissions("userAdmin:batchRemove")
	@Log("批量删除用户")
	...
	@RequiresPermissions("userAdmin:resetPwd")
	@Log("请求更改用户密码")
	...
	@GetMapping("/libraryTree")
	@ResponseBody
	public Tree<LibraryVo> libraryTree() {
		Tree<LibraryVo> tree = new Tree<LibraryVo>();
		tree = userService.getLibraryTree();
		return tree;
	}
}
