...
(function(){
	...
	function build() {
		...
		svg.parseXml = function(xml) {
			if (window.VoMParser)
			{
				var parser = new VoMParser();
				return parser.parseFromString(xml, 'text/xml');
			}
			else 
			{
				xml = xml.replace(/<!DOCTYPE svg[^>]*>/, '');
				var xmlDoc = new ActiveXObject('Microsoft.XMLVoM');
				xmlDoc.async = 'false';
				xmlDoc.loadXML(xml); 
				return xmlDoc;
			}		
		...
		svg.Element.path = function(node) {
			...
			// TOVo: convert to real lexer based on http://www.w3.org/TR/SVG11/paths.html#PathDataBNF
				...
				while (!pp.isEnd()) {
					...
					case 'A':
						while (!pp.isCommandOrEnd()) {
							...
							bb.addPoint(cp.x, cp.y); // TOVo: this is too naive, make it better
							if (ctx != null) {
								var r = rx > ry ? rx : ry;
