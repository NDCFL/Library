...
$('#mytab').bootstrapTable({
    ...
    url: "/searchBook/list",//要请求数据的文件路径
    ...
    columns: [
        {
            ...
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="searchBook"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/searchBook/list'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/searchBook/list'});
}
...
$("#updateSta").click(function () {
    layer.confirm('确认要执行批量修改状态吗？',function(index){
        $.post(
            "/searchBook/deleteManySearchBook",
            {
                "manyId":$("#statusId").val(),
                "status":$("#status").val()
            },
